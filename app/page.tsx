...
type BuiltInMaterial = "Gloss" | "Metal" | "Glass" | "Wood" | "Stone" | "Marble" | "Leather" | "Concrete" | "Rubber" | "Clay" | "Chrome" | "ASCII" | "VGPU Glass" | "VGPU Prism" | "VGPU Holographic";
...
const defaultShapeParams:ShapeParams={geometryMode:"Extrude",thickness:42,segments:18,surfaceDetail:3,edge:24,mass:0,revolveAngle:360,revolveEdge:"Outer",inflateAmount:55,inflateDirection:"Outward",bend:0,bulge:0,taper:0,twist:0,fontWeight:400,letterSpacing:0,lineSpacing:118,material:"Gloss",color:"#E0E0E0",colorOpacity:100,roughness:18,textureRepeat:2,textureRotation:0,textureTint:0,normalStrength:135,glassIor:1.5,glassTransparency:88,vgpuRayAngle:35,vgpuRayStrength:85,asciiCharacters:128,asciiCharacterSet:"Letters + Numbers",asciiCustomSet:" .:-=+*#%@",effect:"None",effectIntensity:70,effectBackground:false,duotoneDarkColor:"#090C13",duotoneColor:"#FF7A45"};
...
const ru:Record<string,string>={
  ...
  "Gloss":"Глянец","Metal":"Металл","Glass":"Стекло","Wood":"Дерево","Stone":"Камень","Marble":"Мрамор","Leather":"Кожа","Concrete":"Бетон","Rubber":"Резина","Clay":"Глина","Chrome":"Хром","VGPU Glass":"VGPU Стекло","VGPU Prism":"VGPU Призма","VGPU Holographic":"VGPU Голографический","GPU refraction":"GPU-рефракция","Spectral glass":"Спектральное стекло","Animated foil":"Анимированная фольга","VGPU Refraction":"VGPU Рефракция","VGPU Bloom":"VGPU Свечение","VGPU Filmic":"VGPU Кино","Ray strength":"Сила луча","Ray angle":"Угол луча",
...
const materials: { name: BuiltInMaterial; note: string }[] = [
  ...
  { name: "VGPU Prism", note: "Spectral glass" },
...
function Home() {
  ...
  const [vgpuRayAngle,setVgpuRayAngle]=useState(35);
  const [vgpuRayStrength,setVgpuRayStrength]=useState(85);
        ...
        <aside className="panel properties-panel">
          ...
          <details className="property-section material-section" open>
            ...
            <div className="section-body">
              ...
              {(material==="Glass"||material==="VGPU Glass")&&<div className="texture-controls glass-controls"><div className="texture-credit"><span>{material==="VGPU Glass"?"VGPU":"GLASS"}</span> {t("Physical refraction")}</div><RangeControl label={t("Refraction (IOR)")} value={glassIor} min={1} max={2.33} step={.01} onChange={setGlassIor}/><RangeControl label={t("Transparency")} value={glassTransparency} min={0} max={100} suffix="%" onChange={setGlassTransparency}/>{material==="VGPU Glass"&&<><RangeControl label={t("Ray strength")} value={vgpuRayStrength} min={0} max={150} suffix="%" onChange={setVgpuRayStrength}/><RangeControl label={t("Ray angle")} value={vgpuRayAngle} min={0} max={360} suffix="°" onChange={setVgpuRayAngle}/></>}</div>}
