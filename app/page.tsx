...
type BuiltInMaterial = "Gloss" | "Metal" | "Glass" | "Wood" | "Stone" | "Marble" | "Leather" | "Concrete" | "Rubber" | "Clay" | "Chrome" | "ASCII" | "VGPU Glass" | "VGPU Holographic" | "VGPU Lava";
type Material = BuiltInMaterial | `Custom:${string}`;
type GeometryMode = "Extrude" | "Revolve" | "Inflate";
type RevolveEdge = "Outer" | "Inner";
type InflateDirection = "Outward" | "Inward";
type PostEffect = "None" | "Cartoon" | "Sketch" | "Halftone" | "Pixelate" | "Chromatic" | "Duotone" | "Dither" | "Glow" | "VGPU Refraction" | "VGPU Bloom" | "VGPU Filmic";
type AsciiCharacterSet = "Letters" | "Numbers" | "Letters + Numbers" | "Arrows & Chevrons" | "Math & Symbols" | "Custom Set";
type Language = "en" | "ru";
type ShapeParams = { geometryMode:GeometryMode;thickness:number;segments:number;surfaceDetail:number;edge:number;mass:number;revolveAngle:number;revolveEdge:RevolveEdge;inflateAmount:number;inflateDirection:InflateDirection;bend:number;bulge:number;taper:number;twist:number;fontWeight:number;letterSpacing:number;lineSpacing:number;material:Material;color:string;colorOpacity:number;roughness:number;textureRepeat:number;textureRotation:number;textureTint:number;normalStrength:number;glassIor:number;glassTransparency:number;glassReflection:number;glassFrost:number;vgpuRayAngle:number;vgpuRayStrength:number;asciiCharacters:number;asciiCharacterSet:AsciiCharacterSet;asciiCustomSet:string;effect:PostEffect;effectIntensity:number;effectBackground:boolean;duotoneDarkColor:string;duotoneColor:string };
...
const defaultShapeParams:ShapeParams={geometryMode:"Extrude",thickness:42,segments:18,surfaceDetail:3,edge:24,mass:0,revolveAngle:360,revolveEdge:"Outer",inflateAmount:55,inflateDirection:"Outward",bend:0,bulge:0,taper:0,twist:0,fontWeight:400,letterSpacing:0,lineSpacing:118,material:"Gloss",color:"#E0E0E0",colorOpacity:100,roughness:18,textureRepeat:2,textureRotation:0,textureTint:0,normalStrength:135,glassIor:1.5,glassTransparency:88,glassReflection:72,glassFrost:0,vgpuRayAngle:12,vgpuRayStrength:95,asciiCharacters:128,asciiCharacterSet:"Letters + Numbers",asciiCustomSet:" .:-=+*#%@",effect:"None",effectIntensity:70,effectBackground:false,duotoneDarkColor:"#090C13",duotoneColor:"#FF7A45"};
...
const ru:Record<string,string>={
  ...
  "Typography":"Типографика","Font weight":"Начертание","Letter spacing":"Межбуквенное расстояние","Line spacing":"Межстрочное расстояние","Appearance":"Внешний вид","Color":"Цвет","Roughness":"Шероховатость","Color opacity":"Прозрачность цвета","Effects":"Эффекты","Cartoon":"Мультфильм","Sketch":"Скетч","Halftone":"Растр","Pixelate":"Пикселизация","Chromatic":"Хроматический","Duotone":"Дуотон","Dither":"Дизеринг","Glow":"Свечение","Intensity":"Интенсивность","First tone":"Первый тон","Second tone":"Второй тон","Affect background":"Обрабатывать фон","Post-processing is unavailable for ASCII":"Пост-обработка недоступна для ASCII","Lighting":"Освещение","Strength":"Яркость","Ambient":"Окружение","Shadow softness":"Мягкость тени","Shadow opacity":"Прозрачность тени","Depth":"Глубина","Soft shadow":"Мягкая тень","Background":"Фон","Scene":"Сцена","None":"Нет","Custom":"Пользовательский","Upload":"Загрузить","Noir":"Нуар","Sky":"Небо","Sunset":"Закат","Gallery":"Галерея","Acid":"Кислотный",
  "Gloss":"Глянец","Metal":"Металл","Glass":"Стекло","Wood":"Дерево","Stone":"Камень","Marble":"Мрамор","Leather":"Кожа","Concrete":"Бетон","Rubber":"Резина","Clay":"Глина","Chrome":"Хром","VGPU Glass":"VGPU Стекло","VGPU Holographic":"VGPU Голографический","VGPU Lava":"VGPU Лава","GPU refraction":"GPU-рефракция","Animated foil":"Анимированная фольга","Official lava material":"Официальный Lava material","VGPU Refraction":"VGPU Рефракция","VGPU Bloom":"VGPU Свечение","VGPU Filmic":"VGPU Кино","Ray strength":"Сила луча","Ray angle":"Угол луча",
...
const materials: { name: BuiltInMaterial; note: string }[] = [
  ...
  { name: "VGPU Glass", note: "GPU refraction" },
  { name: "VGPU Holographic", note: "Animated foil" },
  { name: "VGPU Lava", note: "Official lava material" },
];
...
const postEffects:{name:PostEffect;symbol:string}[]=[{name:"None",symbol:"×"},{name:"Cartoon",symbol:"◒"},{name:"Sketch",symbol:"✎"},{name:"Halftone",symbol:"⠿"},{name:"Pixelate",symbol:"▦"},{name:"Chromatic",symbol:"RGB"},{name:"Duotone",symbol:"◐"},{name:"Dither",symbol:"░"},{name:"Glow",symbol:"✦"},{name:"VGPU Refraction",symbol:"W"},{name:"VGPU Bloom",symbol:"W"},{name:"VGPU Filmic",symbol:"W"}];
...
function fileFromPasteData(data:DataTransfer){
  ...
  const available=[...files,...itemFiles];
  const vector=available.find(file=>file.type==="image/svg+xml"||/\.svg$/i.test(file.name));
  if(vector)return new File([vector],/\.svg$/i.test(vector.name)?vector.name:`figma-${Date.now()}.svg`,{type:"image/svg+xml"});
  const png=available.find(file=>file.type==="image/png"||/\.png$/i.test(file.name));
  return png?new File([png],/\.png$/i.test(png.name)?png.name:`figma-${Date.now()}.png`,{type:"image/png"}):null;
}

async function fileFromClipboardItems(items:ClipboardItem[]){
  for(const type of ["image/svg+xml","text/plain","text/html","image/png"]){
    for(const item of items){
      if(!item.types.includes(type))continue;
      try{
        const blob=await item.getType(type);
        if(type.startsWith("text/")){
          const svg=svgFileFromText(await blob.text());
          if(svg)return svg;
        }else return new File([blob],`figma-${Date.now()}.${type.endsWith("png")?"png":"svg"}`,{type});
      }catch{/* Try the next clipboard representation. */}
    }
  }
  return null;
}
...
function OptionToggle({label,value,options,onChange,t}:{label:string;value:string;options:string[];onChange:(value:string)=>void;t:(value:string)=>string}){
  return <div className="option-control"><span>{label}</span><div role="group" aria-label={label}>{options.map(option=><button key={option} className={value===option?"active":""} onClick={()=>onChange(option)}>{t(option)}</button>)}</div></div>;
}
...
function TonePalette({label,color,colors,onChange,t}:{label:string;color:string;colors:string[];onChange:(color:string)=>void;t:(value:string)=>string}){
  return <div className="effect-color"><div className="control-row"><span>{t(label)}</span><output>{color}</output></div><div className="tone-palette">{colors.map(item=><button key={item} aria-label={`Set ${label.toLowerCase()} ${item}`} className={color===item?"active":""} style={{background:item}} onClick={()=>onChange(item)}/>)}<label className="custom-color">+<input aria-label={`Custom ${label.toLowerCase()}`} type="color" value={color} onInput={event=>onChange(event.currentTarget.value.toUpperCase())}/></label></div></div>;
}
...
function EmbedViewer(){
  ...
  return <main className="embed-shell">
    <ThreeStage ref={stageRef} source={payload.shape.source} fileName={payload.shape.fileName} text={payload.shape.text} fontUrl={payload.shape.fontUrl} fontWeight={p.fontWeight} letterSpacing={p.letterSpacing} lineSpacing={p.lineSpacing} geometryMode={p.geometryMode} thickness={p.thickness} material={p.material} customMaterialUrl={payload.customMaterialUrl} color={p.color} colorOpacity={p.colorOpacity} glassIor={p.glassIor} glassTransparency={p.glassTransparency} glassReflection={p.glassReflection??72} glassFrost={p.glassFrost??0} vgpuRayAngle={p.vgpuRayAngle??12} vgpuRayStrength={p.vgpuRayStrength??95} roughness={p.roughness} light={s.light} lightX={s.lightX} lightY={s.lightY} lightZ={s.lightZ} ambientLight={s.ambientLight} shadowSoftness={s.shadowSoftness} shadowOpacity={s.shadowOpacity} shadows={s.shadows} segments={p.segments} surfaceDetail={p.surfaceDetail} edge={p.edge} mass={p.mass} revolveAngle={p.revolveAngle??360} revolveEdge={p.revolveEdge??"Outer"} inflateAmount={p.inflateAmount} inflateDirection={p.inflateDirection??"Outward"} bend={p.bend} bulge={p.bulge} taper={p.taper} twist={p.twist} textureRepeat={p.textureRepeat} textureRotation={p.textureRotation} textureTint={p.textureTint} normalStrength={p.normalStrength} asciiCharacters={p.asciiCharacters} asciiGlyphs={glyphs} effect={p.material==="ASCII"?"None":p.effect} effectIntensity={p.effectIntensity} effectBackground={p.effectBackground} duotoneDarkColor={p.duotoneDarkColor} duotoneColor={p.duotoneColor} background={s.background} onReady={applyRotation}/>
...
function Home() {
  ...
  const [revolveEdge,setRevolveEdge]=useState<RevolveEdge>("Outer");
  const [inflateAmount,setInflateAmount]=useState(55);
  const [inflateDirection,setInflateDirection]=useState<InflateDirection>("Outward");
  ...
  const [glassReflection,setGlassReflection]=useState(72);
  const [glassFrost,setGlassFrost]=useState(0);
  const [vgpuRayAngle,setVgpuRayAngle]=useState(12);
  const [vgpuRayStrength,setVgpuRayStrength]=useState(95);
  ...
  const [duotoneDarkColor,setDuotoneDarkColor]=useState("#090C13");
  const [duotoneColor,setDuotoneColor]=useState("#FF7A45");
  ...
  const nativePasteAtRef=useRef(0);
  const pasteFingerprintRef=useRef({value:"",at:0});
  ...
  const currentParams:ShapeParams={geometryMode,thickness,segments,surfaceDetail,edge,mass,revolveAngle,revolveEdge,inflateAmount,inflateDirection,bend,bulge,taper,twist,fontWeight,letterSpacing,lineSpacing,material,color,colorOpacity,roughness,textureRepeat,textureRotation,textureTint,normalStrength,glassIor,glassTransparency,glassReflection,glassFrost,vgpuRayAngle,vgpuRayStrength,asciiCharacters,asciiCharacterSet,asciiCustomSet,effect,effectIntensity,effectBackground,duotoneDarkColor,duotoneColor};
  ...
  useEffect(()=>{
    const importPastedFile=(file:File)=>{
      const now=Date.now(),fingerprint=`${file.type}:${file.size}`;
      if(pasteFingerprintRef.current.value===fingerprint&&now-pasteFingerprintRef.current.at<900)return;
      pasteFingerprintRef.current={value:fingerprint,at:now};
    ...
    const handlePaste=(event:ClipboardEvent)=>{
      ...
      if(!file)return;
      nativePasteAtRef.current=Date.now();
      event.preventDefault();
      importPastedFile(file);
    };
    const handleKeyDown=(event:KeyboardEvent)=>{
      if(event.key.toLowerCase()!=="v"||(!event.metaKey&&!event.ctrlKey)||!navigator.clipboard?.read)return;
      const started=Date.now();
      const fallback=navigator.clipboard.read().then(fileFromClipboardItems).catch(()=>null);
      window.setTimeout(()=>{
        if(nativePasteAtRef.current>=started)return;
        fallback.then(file=>{if(file)importPastedFile(file);});
      },80);
    };
    document.addEventListener("paste",handlePaste,true);
    document.addEventListener("keydown",handleKeyDown,true);
    return()=>{document.removeEventListener("paste",handlePaste,true);document.removeEventListener("keydown",handleKeyDown,true);};
  },[importFile]);
  ...
  function applyParams(params:ShapeParams,suppress=true){if(suppress)suppressHistoryRef.current=true;setGeometryMode(params.geometryMode??"Extrude");setThickness(params.thickness);setSegments(params.segments);setSurfaceDetail(params.surfaceDetail??3);setEdge(params.edge);setMass(params.mass);setRevolveAngle(params.revolveAngle??360);setRevolveEdge(params.revolveEdge??"Outer");setInflateAmount(params.inflateAmount??55);setInflateDirection(params.inflateDirection??"Outward");setBend(params.bend);setBulge(params.bulge);setTaper(params.taper);setTwist(params.twist);setFontWeight(params.fontWeight??400);setLetterSpacing(params.letterSpacing??0);setLineSpacing(params.lineSpacing??118);setMaterial(String(params.material)==="VGPU Prism"?"VGPU Glass":params.material);setColor(params.color);setHexDraft(params.color);setColorOpacity(params.colorOpacity);setRoughness(params.roughness);setTextureRepeat(params.textureRepeat);setTextureRotation(params.textureRotation);setTextureTint(params.textureTint);setNormalStrength(params.normalStrength??135);setGlassIor(params.glassIor);setGlassTransparency(params.glassTransparency);setGlassReflection(params.glassReflection??72);setGlassFrost(params.glassFrost??0);setVgpuRayAngle(params.vgpuRayAngle??12);setVgpuRayStrength(params.vgpuRayStrength??95);setAsciiCharacters(params.asciiCharacters??128);setAsciiCharacterSet(params.asciiCharacterSet??"Letters + Numbers");setAsciiCustomSet(params.asciiCustomSet??" .:-=+*#%@");setEffect(params.effect??"None");setEffectIntensity(params.effectIntensity??70);setEffectBackground(params.effectBackground??false);setDuotoneDarkColor(params.duotoneDarkColor??"#090C13");setDuotoneColor(params.duotoneColor??"#FF7A45");}
  ...
  const resetGeometry = () => { setGeometryMode("Extrude");setThickness(42); setSegments(18); setSurfaceDetail(3); setEdge(24); setMass(0);setRevolveAngle(360);setRevolveEdge("Outer");setInflateAmount(55);setInflateDirection("Outward"); };
  const resetDeform = () => { setBend(0); setBulge(0); setTaper(0); setTwist(0); };
  const resetMaterial = () => { setMaterial("Gloss"); setTextureRepeat(2); setTextureRotation(0); setTextureTint(0); setNormalStrength(135); setGlassIor(1.5); setGlassTransparency(88); setGlassReflection(72); setGlassFrost(0); setVgpuRayAngle(12); setVgpuRayStrength(95); setAsciiCharacters(128); setAsciiCharacterSet("Letters + Numbers"); setAsciiCustomSet(" .:-=+*#%@"); };
  ...
  const resetEffects = () => { setEffect("None");setEffectIntensity(70);setEffectBackground(false);setDuotoneDarkColor("#090C13");setDuotoneColor("#FF7A45"); };
      ...
      <section className="workspace">
        <aside className="panel import-panel">
          ...
          {sourceMode==="image"?<button className="dropzone" onClick={() => fileRef.current?.click()} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); importFile(e.dataTransfer.files[0]); }}>
            <span className="upload-icon">↗</span><strong>{t("Drop your shape")}</strong><small>SVG or PNG · max 12 MB</small><small className="figma-hint">{t("Figma: Copy as SVG or PNG, then paste")}</small>
        ...
        <section className="stage" aria-label="3D preview">
          ...
          <div className="scene">
            ...
            <ThreeStage ref={stageRef} source={source} fileName={fileName} text={activeShape?.kind==="text"?activeShape.text:undefined} fontUrl={activeShape?.kind==="text"?activeShape.fontUrl:undefined} fontWeight={fontWeight} letterSpacing={letterSpacing} lineSpacing={lineSpacing} geometryMode={geometryMode} thickness={thickness} material={material} customMaterialUrl={customMaterialUrl} color={color} colorOpacity={colorOpacity} glassIor={glassIor} glassTransparency={glassTransparency} glassReflection={glassReflection} glassFrost={glassFrost} vgpuRayAngle={vgpuRayAngle} vgpuRayStrength={vgpuRayStrength} roughness={roughness} light={light} lightX={lightX} lightY={lightY} lightZ={lightZ} ambientLight={ambientLight} shadowSoftness={shadowSoftness} shadowOpacity={shadowOpacity} shadows={shadows} segments={segments} surfaceDetail={surfaceDetail} edge={edge} mass={mass} revolveAngle={revolveAngle} revolveEdge={revolveEdge} inflateAmount={inflateAmount} inflateDirection={inflateDirection} bend={bend} bulge={bulge} taper={taper} twist={twist} textureRepeat={textureRepeat} textureRotation={textureRotation} textureTint={textureTint} normalStrength={normalStrength} asciiCharacters={asciiCharacters} asciiGlyphs={asciiGlyphs} effect={material==="ASCII"?"None":effect} effectIntensity={effectIntensity} effectBackground={effectBackground} duotoneDarkColor={duotoneDarkColor} duotoneColor={duotoneColor} background={background} demoSpin={Boolean(activeShape?.demo)} onRotationChange={setRotation} onReady={setTriangles} onLoading={setIsRendering} onError={flash} />
        ...
        <aside className="panel properties-panel">
          ...
          <details className="property-section geometry-section" open>
            ...
            <div className="section-body stack-controls">
              ...
              {geometryMode==="Extrude"&&<RangeControl label={t("Thickness")} value={thickness} min={1} max={300} suffix={language==="ru"?"мм":"mm"} onChange={setThickness}/>}
              {geometryMode==="Revolve"&&<><RangeControl label={t("Angle")} value={revolveAngle} min={0} max={360} suffix="°" onChange={setRevolveAngle}/><OptionToggle label={t("Axis edge")} value={revolveEdge} options={["Outer","Inner"]} onChange={value=>setRevolveEdge(value as RevolveEdge)} t={t}/></>}
              ...
              {geometryMode==="Extrude"&&<RangeControl label={t("Edge")} value={edge} min={0} max={300} suffix="%" onChange={setEdge}/>}
              {geometryMode==="Extrude"&&<RangeControl label={t("Mass")} value={mass} min={0} max={250} suffix="%" onChange={setMass}/>}
              {geometryMode==="Inflate"&&<><RangeControl label={t("Inflation")} value={inflateAmount} min={0} max={200} suffix="%" onChange={setInflateAmount}/><OptionToggle label={t("Direction")} value={inflateDirection} options={["Outward","Inward"]} onChange={value=>setInflateDirection(value as InflateDirection)} t={t}/></>}
            </div>
          </details>
          ...
          <details className="property-section effects-section" open>
            ...
            <div className="section-body effects-body">
              ...
              {material==="ASCII"?<small className="effects-note">{t("Post-processing is unavailable for ASCII")}</small>:effect!=="None"&&<><RangeControl label={t("Intensity")} value={effectIntensity} min={0} max={100} suffix="%" onChange={setEffectIntensity}/>{effect==="VGPU Refraction"&&<><RangeControl label={t("Ray strength")} value={vgpuRayStrength} min={0} max={150} suffix="%" onChange={setVgpuRayStrength}/><RangeControl label={t("Ray angle")} value={vgpuRayAngle} min={0} max={360} suffix="°" onChange={setVgpuRayAngle}/></>}{effect==="Duotone"&&<div className="duotone-palettes"><TonePalette label="First tone" color={duotoneDarkColor} colors={paletteColors} onChange={setDuotoneDarkColor} t={t}/><TonePalette label="Second tone" color={duotoneColor} colors={paletteColors} onChange={setDuotoneColor} t={t}/></div>}<label className="switch-row"><span>{t("Affect background")}</span><input type="checkbox" checked={effectBackground} onChange={event=>setEffectBackground(event.target.checked)}/><i/></label></>}
            </div>
          </details>
          ...
          <details className="property-section material-section" open>
            ...
            <div className="section-body">
              ...
              {(material==="Glass"||material==="VGPU Glass")&&<div className="texture-controls glass-controls"><div className="texture-credit"><span>{material==="VGPU Glass"?"VGPU":"GLASS"}</span> {t("Physical refraction")}</div><RangeControl label={t("Refraction (IOR)")} value={glassIor} min={1} max={2.33} step={.01} onChange={setGlassIor}/><RangeControl label={t("Transparency")} value={glassTransparency} min={0} max={100} suffix="%" onChange={setGlassTransparency}/><RangeControl label={t("Reflection")} value={glassReflection} min={0} max={100} suffix="%" onChange={setGlassReflection}/><RangeControl label={t("Frost")} value={glassFrost} min={0} max={100} suffix="%" onChange={setGlassFrost}/>{material==="VGPU Glass"&&<><RangeControl label={t("Ray strength")} value={vgpuRayStrength} min={0} max={150} suffix="%" onChange={setVgpuRayStrength}/><RangeControl label={t("Ray angle")} value={vgpuRayAngle} min={0} max={360} suffix="°" onChange={setVgpuRayAngle}/></>}</div>}
