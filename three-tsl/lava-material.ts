...
export interface LavaMaterial {
  ...
  readonly glowIntensity: ReturnType<typeof uniform>;
  /** Spatial frequency of the lava field on the mesh. */
  readonly scale: ReturnType<typeof uniform>;
}
...
export function createLavaMaterial(options: LavaMaterialOptions): LavaMaterial {
  ...
  // The pulled example targets Three's newer ShaderNodeObject type, which is
  // absent from the versioned @types/three package used by this app.
  const t = (options.timeNode ?? time) as any;
